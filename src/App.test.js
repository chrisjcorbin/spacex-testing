...
// Step 1 - mock the function that makes the API call
jest.mock("./api/fetchMissions"); // mocks ALL exports coming from the file
...
test("render missions when API is called", async () => {
    // Step 2 - mock the data that gets returned by the API
    mockFetchMissions.mockResolvedValueOnce(missionsData);

    render(<App />);
    // run queries and assertions

    const button = screen.getByRole("button", { name: /get data/i });
    userEvent.click(button); // result in an async operation being initiated

    // handle the async operation (Wait for the async operation to finish)
    await waitFor(() => screen.getAllByTestId(/missions/i));
    // JS engine won't run any code below here until the promise is resolved

    expect(screen.getAllByTestId(/missions/i)).toHaveLength(3);
})